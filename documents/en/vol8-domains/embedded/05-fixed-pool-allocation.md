...
translation:
  source: documents/vol8-domains/embedded/05-fixed-pool-allocation.md
  source_hash: 8b9cae0cba3e87028ed2a02b162146a938cd0ecd36d0a8fe88f4f7b0769e2692
  translated_at: '2026-05-26T12:20:31.351834+00:00'
  engine: anthropic
  token_count: 696
---
# Embedded C++ Tutorial: Slab / Arena Implementation and Comparison

Here we begin covering the remaining memory allocation topics using fixed pools, slabs, and arenas — specifically **Slab** and **Arena (Bump / Region)**. While some of this content technically belongs to the operating system level, it never hurts to know it!
...
- **Arena (Region)**: Ideal for short-lived objects or one-off allocation scenarios (e.g., parsing, startup phases), fast allocation (simply moving a pointer), with bulk deallocation done all at once `reset()`.
...
Simply put, we define several size-classes (e.g., 16B, 32B, 64B, 128B...), and each size-class manages multiple slabs. A slab is a contiguous block of memory divided into N object slots. A slab itself can be in one of three states: `empty` (empty), `partial` (partially used), or `full` (full). When allocating, we take a slot from a `partial` or `empty` slab; when deallocating, we add it back to the slab's free list.

This might not seem like much. However, we can now implement adjacent merging so that memory isn't scattered all over the place. We can also quickly match the nearest size, and since everything is pooled, we can perform specialized optimizations for different object types (construction/destruction caching, debug headers).

#### A Simplified Version

For the sake of brevity, we will build a simplified slab:
...
- `reset()` resets `head` back to the initial position (bulk deallocation of all allocations)

As a result, arena allocation is extremely fast (pointer arithmetic), highly suited for temporary memory, and produces zero/low fragmentation. However, there are several drawbacks:
...
<OnlineCompilerDemo
  title="Fixed Pool Allocator: O(1) Allocation and Deallocation Demo"
  ...
  description="The fixed pool example can be run directly and is also suitable for observing which instructions remain in the optimized allocation path."
  allow-run
  allow-x86-asm
  allow-arm-asm
/>
