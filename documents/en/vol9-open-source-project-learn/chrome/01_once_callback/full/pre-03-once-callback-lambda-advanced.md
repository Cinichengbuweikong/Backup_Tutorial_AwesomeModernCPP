...
`*this` is the current OnceCallback object, and `std::move(*this)` casts it to an rvalue. The init capture `self = std::move(*this)` triggers OnceCallback's move constructor, moving `func_`, `status_`, and `token_` entirely into the lambda's closure object. After the move, `*this` (the original OnceCallback object) enters a "moved-from" state—`func_` and `token_` are already empty or null.
