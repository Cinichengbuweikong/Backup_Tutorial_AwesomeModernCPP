...
- 17
- 20
description: Exploring the real-world trade-offs between performance and maintainability
...
translation:
  ...
  translated_at: '2026-05-26T10:25:41.290598+00:00'
  engine: anthropic
  token_count: 614
---
# Language Selection Principles: The Real Trade-Off Between Performance and Maintainability

Let me vent a little—I've noticed that many developers lack rational thinking when choosing tools, tending to evaluate them based on labels rather than actual performance. As a result, language selection is often framed as a moral judgment: "C is pure, close-to-hardware real engineering; C++/Rust are high-level, lazy productivity tools." This rhetoric is emotionally charged, but it's completely useless for engineering decisions. The real engineering question is never "which language is more advanced," but rather "for this specific MCU, this timing constraint, this team, and this product roadmap, which approach lets us build things correctly and completely at a reasonable cost?" Performance and maintainability are not inherently opposing magical forces; they introduce different risks and overheads across different dimensions. Treating a language as a master key or a silver bullet, rather than as part of an engineering toolchain, is the biggest mistake we can make.

## What "Performance" Actually Means in Embedded

First, let's clarify what "performance" means in this context. Performance isn't just a raw CPU benchmark score; it encompasses flash usage, RAM usage, startup time, real-time interrupt latency, power consumption, and timing predictability. "Performance" means something entirely different when we are running an interrupt service routine (ISR) that must complete within 5µs on a Cortex-M0, compared to running a UI thread on a Raspberry Pi. Similarly, maintainability isn't an abstract concept of "pretty code." It refers to whether the team can stably reuse, fix, and extend this codebase over the next six months to three years: are unit tests easy to write, can bugs be reproduced in CI, and can a new team member understand the module boundaries within a week?
...
`ScopedIrqLock` disables interrupts upon construction and automatically restores them upon destruction—the entire process happens on the stack, involves no heap allocation, and is completely predictable in terms of timing. This way, we maintain high-level maintainability while guaranteeing determinism on the critical path.
...
However, introducing a new language also means the toolchain, compiler support, third-party library maturity, and the team's learning curve must be thoroughly evaluated. No language can automatically handle compliance work for you—a language simply shifts certain errors forward to compile time, reducing runtime risks. Just make sure the math checks out when selecting a language.

## Total Cost of Ownership

Finally, let's discuss long-term maintenance costs. Debugging time, defect fixing, team handoffs, and the difficulty of future extensions—these are the real budget drainers. More often than not, good testing, reproducible build pipelines, clear module boundaries, and thorough documentation have a far greater impact on total cost than saving a few milliseconds on some micro-benchmark. Choosing a language that allows the team to produce stable output, facilitates code reviews, and enables automated testing is often far more meaningful than fighting over that tiny bit of single-core peak performance.
