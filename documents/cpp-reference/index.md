...
| [范围 for (range-for)](core-language/07-range-for.md) | 语言特性 | 容器遍历语法糖 | **高** |
| 移动语义 (右值引用) | 语言特性 | 资源转移而非拷贝 | **高** |
| [std::move / std::forward](core-language/08-move-forward.md) | `<utility>` | 移动与完美转发工具 | **高** |
| [nullptr](core-language/04-nullptr.md) | 语言特性 | 类型安全的空指针常量 | **高** |
| [enum class](core-language/05-enum-class.md) | 语言特性 | 作用域强类型枚举 | **高** |
| [override / final](core-language/06-override-final.md) | 语言特性 | 虚函数显式标注 | **高** |
| static_assert | 语言特性 | 编译期断言 | **高** |
| [可变参数模板](templates/02-variadic-templates.md) | 语言特性 | 任意数量模板参数 | **高** |
| [std::initializer_list](containers/05-initializer-list.md) | `<initializer_list>` | 统一初始化列表 | **高** |
| [std::array](containers/04-array.md) | `<array>` | 编译期固定大小数组 | **高** |
...
| [std::thread](concurrency/02-thread.md) | `<thread>` | 平台无关线程 | **高** |
| [std::mutex / lock_guard](concurrency/03-mutex.md) | `<mutex>` | 互斥锁与 RAII 锁 | **高** |
| [std::atomic](concurrency/01-atomic.md) | `<atomic>` | 无锁原子操作 | **高** |
...
| 特性 | 头文件 | 简述 | 适用性 |
|------|--------|------|--------|
| [std::make_unique](memory/04-make-unique.md) | `<memory>` | 异常安全的 unique_ptr 创建 | **高** |
| [泛型 lambda](core-language/09-generic-lambda.md) | 语言特性 | lambda 参数使用 auto | **高** |
| 返回类型推导 (auto return) | 语言特性 | 函数返回值 auto 推导 | **中** |
| constexpr 扩展 | 语言特性 | 放宽 constexpr 限制（循环/局部变量） | **高** |
| decltype(auto) | 语言特性 | 完美转发返回类型推导 | **中** |
| [std::exchange](core-language/10-exchange.md) | `<utility>` | 替换并返回旧值 | **中** |
...
| [std::filesystem](containers/06-filesystem.md) | `<filesystem>` | 文件系统操作 | **中** |
| [结构化绑定](core-language/11-structured-binding.md) | 语言特性 | 多返回值解构 | **高** |
| [if constexpr](core-language/13-if-constexpr.md) | 语言特性 | 编译期条件分支 | **高** |
| [折叠表达式](templates/03-fold-expressions.md) | 语言特性 | 参数包展开运算 | **高** |
| CTAD | 语言特性 | 类模板参数推导 | **高** |
| 保证拷贝消除 | 语言特性 | 强制消除临时对象拷贝 | **高** |
| std::invoke | `<functional>` | 统一调用接口 | **中** |
| std::apply | `<tuple>` | tuple 展开为函数参数 | **中** |
| [内联变量](core-language/14-inline-variables.md) | 语言特性 | 头文件中定义全局变量 | **中** |
| std::byte | `<cstddef>` | 独立字节类型 | **中** |
| std::pmr 内存资源 | `<memory_resource>` | 多态分配器内存资源 | **中** |
| std::shared_mutex | `<shared_mutex>` | 读写锁 | **中** |
| [嵌套命名空间 (A::B::C)](core-language/15-nested-namespace.md) | 语言特性 | 命名空间简写 | **低** |
...
| [std::format](containers/07-format.md) | `<format>` | 类型安全的格式化输出 | **高** |
| [std::jthread](concurrency/04-jthread.md) | `<thread>` | 自动 join 的线程类 | **高** |
| [三路比较 (<=>)](core-language/12-spaceship-operator.md) | `<compare>` | 统一比较运算符 | **高** |
| [Coroutines](core-language/16-coroutines.md) | `<coroutine>` | 无栈协程 | **高** |
| [Modules](core-language/17-modules.md) | 语言特性 | 替代头文件的编译单元 | **高** |
...
| 特性 | 头文件 | 简述 | 适用性 |
|------|--------|------|--------|
| [std::expected](memory/05-expected.md) | `<expected>` | 错误处理包装类型 | **中** |
...
| [std::make_unique](memory/04-make-unique.md) | C++14 | `<memory>` | 异常安全的 unique_ptr 创建 | **高** |
| [std::optional](memory/03-optional.md) | C++17 | `<optional>` | 可选值包装 | **高** |
| std::pmr 内存资源 | C++17 | `<memory_resource>` | 多态分配器内存资源 | **中** |
| [std::expected](memory/05-expected.md) | C++23 | `<expected>` | 错误处理包装类型 | **中** |
...
| [std::array](containers/04-array.md) | C++11 | `<array>` | 编译期固定大小数组 | **高** |
...
| [std::filesystem](containers/06-filesystem.md) | C++17 | `<filesystem>` | 文件系统操作 | **中** |
| Ranges | C++20 | `<ranges>` | 组合式范围与视图 | **高** |
| [std::span](containers/01-span.md) | C++20 | `<span>` | 连续序列的非拥有视图 | **高** |
| [std::format](containers/07-format.md) | C++20 | `<format>` | 类型安全的格式化输出 | **高** |
...
| 特性 | 版本 | 头文件 | 简述 | 适用性 |
|------|------|--------|------|--------|
| [std::thread](concurrency/02-thread.md) | C++11 | `<thread>` | 平台无关线程 | **高** |
| [std::mutex / lock_guard](concurrency/03-mutex.md) | C++11 | `<mutex>` | 互斥锁与 RAII 锁 | **高** |
| [std::atomic](concurrency/01-atomic.md) | C++11 | `<atomic>` | 无锁原子操作 | **高** |
...
| [std::jthread](concurrency/04-jthread.md) | C++20 | `<thread>` | 自动 join 的线程类 | **高** |
...
| [范围 for](core-language/07-range-for.md) | C++11 | 语言特性 | 容器遍历语法糖 | **高** |
| 移动语义 (右值引用) | C++11 | 语言特性 | 资源转移而非拷贝 | **高** |
| [std::move / std::forward](core-language/08-move-forward.md) | C++11 | `<utility>` | 移动与完美转发工具 | **高** |
| [nullptr](core-language/04-nullptr.md) | C++11 | 语言特性 | 类型安全的空指针 | **高** |
| [enum class](core-language/05-enum-class.md) | C++11 | 语言特性 | 作用域强类型枚举 | **高** |
| [override / final](core-language/06-override-final.md) | C++11 | 语言特性 | 虚函数显式标注 | **高** |
...
| [泛型 lambda](core-language/09-generic-lambda.md) | C++14 | 语言特性 | lambda 参数使用 auto | **高** |
...
| [结构化绑定](core-language/11-structured-binding.md) | C++17 | 语言特性 | 多返回值解构 | **高** |
| [if constexpr](core-language/13-if-constexpr.md) | C++17 | 语言特性 | 编译期条件分支 | **高** |
| CTAD | C++17 | 语言特性 | 类模板参数推导 | **高** |
| 保证拷贝消除 | C++17 | 语言特性 | 强制消除临时对象拷贝 | **高** |
| [内联变量](core-language/14-inline-variables.md) | C++17 | 语言特性 | 头文件定义全局变量 | **中** |
| std::byte | C++17 | `<cstddef>` | 独立字节类型 | **中** |
| [嵌套命名空间](core-language/15-nested-namespace.md) | C++17 | 语言特性 | A::B::C 简写 | **低** |
| if/switch 初始化语句 | C++17 | 语言特性 | 条件语句内变量声明 | **中** |
| [三路比较 (<=>)](core-language/12-spaceship-operator.md) | C++20 | `<compare>` | 统一比较运算符 | **高** |
| [Coroutines](core-language/16-coroutines.md) | C++20 | `<coroutine>` | 无栈协程 | **高** |
| [Modules](core-language/17-modules.md) | C++20 | 语言特性 | 替代头文件 | **高** |
...
| [deducing this](core-language/18-deducing-this.md) | C++23 | 语言特性 | 显式对象参数推导 | **中** |
...
| [可变参数模板](templates/02-variadic-templates.md) | C++11 | 语言特性 | 任意数量模板参数 | **高** |
| [std::initializer_list](containers/05-initializer-list.md) | C++11 | `<initializer_list>` | 统一初始化列表 | **高** |
| std::integer_sequence | C++14 | `<utility>` | 编译期整数序列 | **中** |
| [折叠表达式](templates/03-fold-expressions.md) | C++17 | 语言特性 | 参数包展开运算 | **高** |
