...
- **难度 · 前置**：中级 / 卷一、卷二。
- **建议节奏**：篇幅小而深，按需精读；做性能敏感或嵌入式开发时回头看。全卷已完工，`vector`/`string`/`char8_t` 等篇质量最稳，可作入口。
...
- **难度 · 前置**：高级 / 卷二、卷三。
- **建议节奏**：先读协程、Ranges、三路比较三块；模板基础、concepts 体系、反射三块目前是空白，正在规划，随用随补。
...
- **难度 · 前置**：中高 / 卷一 ~ 卷四。
- **建议节奏**：全仓投入最重、动手项目最多的一卷，配有 Lab 0–5 + Capstone（Mini Concurrent Runtime）。强烈建议动手做 Lab，别只读。目前 Lab 0/1 有完整脚手架代码可直接上手，Lab 2–5 与 Capstone 暂只有手册文字、脚手架待补。
...
- **难度 · 前置**：中级 / C++ 基础。
- **建议节奏**：作为卷七的前置；做嵌入式/交叉编译前必读。内容主题齐全，但仍是早期博客搬运稿，待按项目声音重写打磨。
...
- ✓ **成熟稳定**：卷二 现代特性、卷三 标准库、卷五 并发。
- ✦ **推进中**：卷一 基础（正重写为全栈入门）、卷七 工程、卷八 领域（嵌入式主线已完整）、卷九 开源研读、卷十 课程笔记。
- ◇ **扩充 / 重写中**：编译与链接深入（博客搬运待重写）、卷四 高级（模板/concepts/反射三块空白）、卷六 性能（扩充中）。
